...
import { Container, Form, Button, Alert, Card, Row, Col, ListGroup, Spinner } from 'react-bootstrap';
        ...
        if (editClassId) {
            ...
            try {
                const response = await axios.put(
                    `http://localhost:5000/api/live-classes/${editClassId}`,
            ...
            try {
                const response = await axios.post(
                    'http://localhost:5000/api/add-live-class',
                    { title, videoId, description, thumbnail, allowedUsers: selectedUsers },
                    { headers: { Authorization: `Bearer ${localStorage.getItem('token')}` } }
                );
